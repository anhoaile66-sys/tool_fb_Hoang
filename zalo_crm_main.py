...
IMAGE_FILE_DATA_PATH = f"{ZALO_IMAGE_PATH}Zalo_image_file_data"
AVA_NUMBER_PATH = f"{ZALO_BASE_PATH}Zalo_image_file_data/ava.txt"
MES_NUMBER_PATH = f"{ZALO_BASE_PATH}Zalo_image_file_data/mes.txt"
file_lock = Lock()
...
NUM_PHONE_ZALO_FILE = "{ZALO_BASE_PATH}num_phone_zalo.txt"
...
id_port = {
    ...
    "22615833": [{"A09": "9PAM7DIFW87DOBEU"}, {"A06": "CEIN4X45I7ZHFEFU"}], #Chị Ngô Dung
    "22814414": [{"A19": "Z5LVOF4PRGXGTS9H"}, {"A02": "7DXCUKKB6DVWDAQO"}], #Chị Bích Ngọc
    "22789191": [{"A14": "F6NZ5LRKWWGACYQ8"}, {"A01": "R8YY70F5MKN"}], #Chị Lại Thị Nhàn
    "22833463": [{"A15": "QK8TEMKZMBYHPV6P"}, {"A03": "TSPNH6GYZLPJBY6X"}], #Chị Thư
    "22636101":[{"A16": "IJP78949G69DKNHM"}, {"A05": "UWJJOJLB85SO7LIZ"}], #Chị Thùy
    "22896992":[{"A13": "EY5H9DJNIVNFH6OR"}, {"A04": "7HYP4T4XTS4DXKCY"}], #Huyền Trang
    ...
    try:
        ...
        while len(friends) < max_friends:
            ...
            for id in range(num_item):
                ...
                name = raw.split("\n", 1)[0].strip()
                if not name or name in seen or name in friend_name or "Tài khoản" in name or raw == "Zalo":
                    continue
                try:
...
def get_list_members_group_u2(d: u2.Device, max_scroll: int = 100, scroll_delay: float = 1.0):
    ...
    try:
        ...
        d.swipe_ext(Direction.FORWARD, scale=0.2)
        eventlet.sleep(1.0)
        d.xpath(
        "//android.widget.FrameLayout[contains(@text, 'Xem thành viên')]").click()
    eventlet.sleep(1.0)
...
def get_list_users_new():
    ...
    for dp in device_and_port:
        ...
        device_id = dp[port]
        try:
            with open(f'{ZALO_BASE_PATH}Zalo_data_login_path_{device_id}.json', 'r', encoding='utf-8') as f:
                zalo_data = json.load(f)
            with open(f'{ZALO_BASE_PATH}device_status_{device_id}.json', 'r') as f:
            ...
            for zalo in zalo_data:
                ...
                user_db.append({"num_phone_zalo": zalo['num_phone_zalo'], "connect": connect, "name_device": port, "status": zalo['status'], "user_name": zalo['name'], "avatar": ava, "port": port})
        except Exception as e:
            print(e)
            pass
    if len(user_db) == 0:
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            if check_busy:
                ...
                try:
                    ...
                    try:
                        #        print(document)
                        with open(f'{ZALO_BASE_PATH}Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'r', encoding='utf-8') as f:
                            data = json.load(f)
                        for id in range(len(data)):
                            #            print(document[domain])
...
#        print(data)
                        with open(f'{ZALO_BASE_PATH}Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'w', encoding='utf-8') as f:
                            json.dump(data, f, ensure_ascii=False, indent=4)
                            print(
        ...
        if not doc['status']:
            ...
            for it in docs:
                if it['status']:
                    ...
                    break
            try:
                with open(f'{ZALO_BASE_PATH}device_status_{dict_phone_device[num_phone_zalo]}.json', 'r') as f:
                    device_status = json.load(f)
                if not device_status['active']:
                    device_status['active'] = True
                    with open(f"{ZALO_BASE_PATH}device_status_{dict_phone_device[num_phone_zalo]}.json", 'w') as f:
                ...
                if not on_chat:
                    ...
                    if friend_or_not == 'yes':
                        ...
                        if name_ntd not in fr_name:
                            ...
                            list_friend.append({"name": name_ntd, "ava": ava, "phone": num_send_phone_zalo})
                            ck_add_or_dl = True
                ...
                if not on_chat:
                    ...
                    if friend_or_not == 'yes':
                        ...
                        if name not in fr_name:
                            ...
                            if name in dict_new_friend[num_phone_zalo].keys:
                               ava =  dict_new_friend[num_phone_zalo][name]['ava']
                            list_friend.append({"name": name, "ava": ava, "phone": num_send_phone_zalo})
                            ck_add_or_dl = True
                    elif friend_or_not == 'no':
                        for id in range(len(list_friend)):
                            if list_friend[id]['name'] == name:
...
# docs giờ là một list chứa mọi document tìm được
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        ...
        try:
            ...
            if len(dict_queue_device[id_device]) == 0:
                dict_id_chat[id_device] = ""
                return jsonify({"status": "Có lỗi xảy ra"})
        if not doc['status']:
            try:
                docs = get_base_id_zalo_json(
                ZALO_BASE_PATH, "id_device", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {"id_device": id_device})
        ...
        try:
            ...
            if not on_chat:
                ...
                if not check:
                    ...
                    dict_new_friend[num_phone_zalo][name_ntd] = {"phone": num_send_phone_zalo, "ava": avatar_64, "time": "", "friend_or_not": friend_or_not, "message": "", "status": "seen",  "data_chat_box": []}
                if check:
                    data_update = {"num_phone_zalo": num_phone_zalo,
                            "list_prior_chat_boxes": list_prior_chat_boxes}
                    update_base_document_json(
                    ...
                    for k in result[i2].keys():
                        ...
                        for i3 in range(len(result[i2][k])):
                            ...
                            if "image_data" in result[i2][k][i3].keys():
                                ...
                                #print(result[i2][k][i3]['image_data'])
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        ...
        try:
            ...
            if type == 'image':
                ...
                with open(f'{ZALO_BASE_PATH}image{image_number}.jpg', 'wb') as f:
                    f.write(avatar)
                print(
                    f"Đã lưu vào thư mục Zalo_base/image{image_number}.jpg")
                d.push(f'{ZALO_BASE_PATH}image{image_number}.jpg',
        ...
        try:
            ...
            if name_group != "":
                ...
                try:
                    ...
                    d.xpath('//*[@resource-id="com.zing.zalo:id/layout_update_avatar"]/android.widget.LinearLayout[1]').click()
                eventlet.sleep(0.1)
...
def api_log_in_status(id_device):
    ...
    file_path = f"{ZALO_BASE_PATH}device_status_{id_device}.json"
    # global device_connect
    global dict_devices
    ...
    #    with open(f'{ZALO_BASE_PATH}device_status_{id_device}.json', 'r') as f:
    #        old_device_status = json.load(f)
    #    max_mes = old_device_status["max_message_per_day"]
    #    max_add = old_device_status["max_add_friend_per_day"]
    ...
    with open(f"{ZALO_BASE_PATH}device_status_{id_device}.json", 'w') as f:
        json.dump(device_status, f, indent=4)

    file_path_new = f"{ZALO_BASE_PATH}Zalo_data_login_path_{id_device}.json"
    if os.path.exists(file_path_new):
        with open(f'{ZALO_BASE_PATH}Zalo_data_login_path_{id_device}.json', 'r', encoding='utf-8') as f:
            zalo_data = json.load(f)
    ...
    try:
        ...
        #dict_device_and_phone[id_device] = []
        ...
        for id in range(len(name_zalos)):
            ...
            with open(f"{ZALO_BASE_PATH}Zalo_data_login_path_{id_device}.json", 'w', encoding="utf-8") as f:
        ...
        # for num_phone_zalo in phone:
            ...
            if has_first_update and len(result4) > 0 and len(old_list_prior_chat_boxes) > 0:
                try:
                    to_remove = {(old.get('name'), old.get('message')) for old in old_list_prior_chat_boxes}
                    result4 = [new for new in result4 if (new.get('name'), new.get('message')) not in to_remove]            
                except Exception as e:
                    print(e)
...
def background_update_1vs1_loop(id_device):
    ...
    while True:
        ...
        try:
            document = get_base_id_zalo_json(ZALO_BASE_PATH, "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo[id_device]]}", {
        ...
        try:
            with open(f'{ZALO_BASE_PATH}device_status_{dict_phone_device[now_phone_zalo[id_device]]}.json', 'r') as f:
                device_status = json.load(f)
            if not device_status['update']:
        ...
        try:
            documents = get_base_id_zalo_json(
                ZALO_BASE_PATH, "id_device", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo[id_device]]}", {"id_device": id_device})
        except Exception as e:
            print(e)
            continue
        ...
        while True:
            ...
            if btn.exists:
                ...
                try:
                    ...
                    #print(name_ntd)
                    try:
                        with open(f'{ZALO_BASE_PATH}device_status_{dict_phone_device[curr_phone_zalo]}.json', 'r') as f:
                            device_status = json.load(f)
                    except Exception as e:
