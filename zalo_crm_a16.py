...
try:
    ...
    print(e)
db = client["D:/Zalo_CRM/Zalo_base"]
...
def get_list_friends_u2(d: u2.Device, max_friends: int = 150, scroll_delay: float = 0.8, retire=3):
    """
...
def get_list_groups_u2(d: u2.Device, max_groups: int = 50, scroll_delay: float = 0.8, retire=3):
    """
...
def get_list_invite_friends_u2(d: u2.Device, max_friends: int = 100, scroll_delay: float = 0.8, retire=3):
    """
...
def get_list_members_group_u2(d: u2.Device, max_scroll: int = 100, scroll_delay: float = 0.8):
...
def handle_join(data):
    ...
    # res =  update_port_base_id_chat("D:/Zalo_CRM/Zalo_base", "Zalo_data_login_port",room)
...
def api_get_list_friend(data_body):
    ...
    docs = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            docs = get_base_id_zalo_json(
                "D:/Zalo_CRM/Zalo_base", "id_device", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {"id_device": id_device})
            check_busy = False
            for doc in docs:
            ...
            if check_busy:
                ...
                one = time.time()
                doc = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
                ...
                if not doc['status']:
                    ...
                    for it in docs:
                        if it['status']:
                            ...
                            break
                    if current_phone != "":
                        status = update_base_document_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
            ...
            #     json.dump(dict_zalo_online, json_file, indent=4)
                    ...
                    update_base_document_json(
                        "D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", data_update)
...
def get_list_prior_chat_boxes_u2(d: u2.Device, max_chat_boxes: int = 50, scroll_delay: float = 0.8, retire=3, scroll_or_not=True):
    """
    Lấy toàn bộ nhóm từ tab Danh bạ trên Zalo Android bằng uiautomator2.
    - d: uiautomator2 Device
    - max_groups: giới hạn tối đa số nhóm thu thập
...
def get_list_unseen_chat_boxes_u2(d: u2.Device, max_chat_boxes: int = 50, scroll_delay: float = 0.8, retire=3):
    """
    Lấy toàn bộ nhóm từ tab Danh bạ trên Zalo Android bằng uiautomator2.
    - d: uiautomator2 Device
    - max_groups: giới hạn tối đa số nhóm thu thập
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            if check_busy:
                ...
                try:
                    ...
                    try:
                        #        print(document)
                        with open(f'D:/Zalo_CRM/Zalo_base/Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'r', encoding='utf-8') as f:
                            data = json.load(f)
                        for id in range(len(data)):
                            #            print(document[domain])
...
#        print(data)
                        with open(f'D:/Zalo_CRM/Zalo_base/Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'w', encoding='utf-8') as f:
                            json.dump(data, f, ensure_ascii=False, indent=4)
                            print(
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                with open(f'D:/Zalo_CRM/Zalo_base/device_status_{dict_phone_device[num_phone_zalo]}.json', 'r') as f:
                    device_status = json.load(f)
                if not device_status['active']:
                    device_status['active'] = True
                    with open(f"D:/Zalo_CRM/Zalo_base/device_status_{dict_phone_device[num_phone_zalo]}.json", 'w') as f:
                ...
                if num_send_phone_zalo != "":
                    ...
                    if not on_chat:
                        ...
                        if not check_ex:
                            ...
                            try:
                                chat_num = d(
                                    resourceId="com.zing.zalo:id/btn_search_result", text=f"{name}\n")
                                chat_num.click_exists(timeout=1)
                                # eventlet.sleep(0.1)
...
def safe_click(d, cx, cy, unwanted_resid="com.zing.zalo:id/chat_layout_group_topic",
    ...
    if el.exists:
        try:
            ...
            # w, h = d.window_size()
            alt_x, alt_y = cx, cy
            d.long_click(alt_x, alt_y, duration=0.8)
            return d
        ...
        if top <= cy <= bottom:
            new_x = cx
            new_y = bottom + 5
            d.long_click(new_x, new_y, duration=0.8)
            return d
...
def get_data_chat_boxes_u2(d: u2.Device, gr_or_pvp: str, time_and_mes, max_scroll: int = 5, scroll_delay: float = 0.8):
    """
...
def get_data_chat_boxes_1vs1_u2(d: u2.Device, time_and_mes, max_scroll: int = 5, scroll_delay: float = 0.8):
    """
    Trả về dict chứa hộp chat hiện có trong màn hình chat của Zalo Android.
    - div_child_selector: dict để tìm tab/conversation (ví dụ {'resourceId': '...', 'text': 'Tên bạn'})
    - device_url: đường dẫn để kết nối uiautomator2 (ví dụ '192.168.1.100:5555')
...
def get_data_chat_boxes_gr_u2(d: u2.Device, time_and_mes, list_mems, max_scroll: int = 5, scroll_delay: float = 0.8):
    """
    Trả về dict chứa hộp chat hiện có trong màn hình chat của Zalo Android.
    - div_child_selector: dict để tìm tab/conversation (ví dụ {'resourceId': '...', 'text': 'Tên bạn'})
    - device_url: đường dẫn để kết nối uiautomator2 (ví dụ '192.168.1.100:5555')
        ...
        if dict_status_zalo[num_phone_zalo] != '':
            ...
            if check_busy:
                ...
                if not document['status']:
                     docs = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "id_device", "Zalo_data_login_path", {"id_device": id_device})
                     current_phone = ""
                     for it in docs:
                        if it['status']:
                            current_phone = it['num_phone_zalo']
                            break
                     if current_phone != "":
                        status = update_base_document_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", "Zalo_data_login_path", {"num_phone_zalo": current_phone, "status" : False})
                     d = switch_account(d, user_name)
                     status = update_base_document_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", "Zalo_data_login_path", {"num_phone_zalo": num_phone_zalo, "status" : True})
                '''
                # dict_status_zalo[num_phone_zalo] = "get_data_one_box_chat"
                if 'list_prior_chat_boxes' not in document.keys():
                    document['list_prior_chat_boxes'] = []
        ...
        if dict_status_zalo[num_phone_zalo] != '':
            ...
            if check_busy:
                ...
                if not check:
                    ...
                    print("Hả thật vậy sao")
                    print("Đoạn chat hiện tại", type(list_prior_chat_boxes[pick]))
                    if 'data_chat_box' not in list_prior_chat_boxes[pick].keys() or list_prior_chat_boxes[pick]['data_chat_box'] == []:
                        print(list_prior_chat_boxes[pick].keys())
                        print(list_prior_chat_boxes[pick]['data_chat_box'])
                        print(list_prior_chat_boxes[pick])
                        ...
                        time_and_mes = {
                            "time": last_data_mes['time'], "message": last_data_mes['data']}
                raw_result, lst_time_str, lst_message = get_data_chat_boxes_gr_u2(
                ...
                if len(result) > 0:
                    for res in result:
                        list_prior_chat_boxes[pick]['data_chat_box'].append(res)
                    list_prior_chat_boxes[pick]['time'] = lst_time_str
                    list_prior_chat_boxes[pick]['message'] = lst_message
                    list_prior_chat_boxes[pick]['status'] = "unseen"
                    list_prior_chat_boxes.insert(
        ...
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                if type == 'image':
                    ...
                    with open(f'D:/Zalo_CRM/Zalo_base/image{image_number}.jpg', 'wb') as f:
                        f.write(avatar)
                    print(
                        f"Đã lưu vào thư mục Zalo_base/image{image_number}.jpg")
                    d.push(f'D:/Zalo_CRM/Zalo_base/image{image_number}.jpg',
                           f'/sdcard/Download/image{image_number}.jpg')
                    # d.push('D:/Zalo_CRM/Zalo_base/image.jpg', f'/sdcard/DCIM/Zalo/image{image_number}.jpg')
                    # d.push('D:/Zalo_CRM/Zalo_base/image.jpg', f'/sdcard/Pictures/Zalo/image{image_number}.jpg')
                    eventlet.sleep(0.1)
                    image_number -= 1
                    ...
                    if "word" in file_type:
                        with open(f'D:/Zalo_CRM/Zalo_base/{file_name}.docx', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'D:/Zalo_CRM/Zalo_base/{file_name}.docx', f'/sdcard/Download/{file_name}.docx')
                    elif "pdf" in file_type:
                        with open(f'D:/Zalo_CRM/Zalo_base/{file_name}.pdf', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'D:/Zalo_CRM/Zalo_base/{file_name}.pdf', f'/sdcard/Download/{file_name}.pdf')
                    elif "sheet" in file_type:
                        with open(f'D:/Zalo_CRM/Zalo_base/{file_name}.xlsx', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'D:/Zalo_CRM/Zalo_base/{file_name}.xlsx', f'/sdcard/Download/xlsx{image_number}.xlsx')
                    elif "present" in file_type:
                        with open(f'D:/Zalo_CRM/Zalo_base/present{image_number}.ppt', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'D:/Zalo_CRM/Zalo_base/present{image_number}.ppt', f'/sdcard/Download/present{image_number}.ppt')
                    '''
                    with open(f'D:/Zalo_CRM/Zalo_base/{file_name}', 'wb') as f:
                        f.write(file_decode)
                    d.push(f'D:/Zalo_CRM/Zalo_base/{file_name}',
                ...
                if True:
                    ...
                    if not on_chat:
                        ...
                        if not check_ex:
                            ...
                            try:
                                chat_num = d(
                                    resourceId="com.zing.zalo:id/btn_search_result", text=f"{name_share}\n")
                                chat_num.click_exists(timeout=1)
                                # eventlet.sleep(0.1)
                                print("Nhấn được chat  chưa")
                            except Exception:
        ...
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                try:
                    d.app_start("com.zing.zalo", stop=True)
                    eventlet.sleep(0.8)
                    d(resourceId="com.zing.zalo:id/action_bar_plus_btn").click_exists(timeout=1)
                    eventlet.sleep(0.1)
                    d(text="Tạo nhóm").click()
                    eventlet.sleep(0.1)
                    if name_group != "":
                        d.xpath('//*[@resource-id="com.zing.zalo:id/header_section"]/android.widget.LinearLayout[1]').click()
                        eventlet.sleep(0.1)
                    ...
                    for mem in mem_list:
                        ...
                        el = d.xpath('//android.widget.FrameLayout[@index="0" and normalize-space(@text) != ""]')
                        if el.exists:
                            el.click()
                            eventlet.sleep(0.1)
                    ...
                    list_prior_chat_boxes.append({"name": name_group, "phone": "", "ava": avatar, "time": "", "message": "", "status": "seen", "data_chat_box": [], "list_mems": list_mems, "check_mems": check_mems})
                    list_prior_chat_boxes.insert(
                        0, list_prior_chat_boxes.pop(-1))
                    list_group.append({"name": name_group, "ava": avatar, "list_mems": list_mems, "check_mems": check_mems})
                except Exception as e:
                    print(e)
                    dict_status_zalo[num_phone_zalo] =  ""
                    return jsonify({"status": "Tạo nhóm thất bại"})
...
def api_log_in_status(id_device):
    ...
    file_path = f"D:/Zalo_CRM/Zalo_base/device_status_{id_device}.json"
    global device_connect
    ...
    with open(f"D:/Zalo_CRM/Zalo_base/device_status_{id_device}.json", 'w') as f:
        json.dump(device_status, f, indent=4)

    file_path_new = f"D:/Zalo_CRM/Zalo_base/Zalo_data_login_path_{id_device}.json"
    if os.path.exists(file_path_new):
        with open(f'D:/Zalo_CRM/Zalo_base/Zalo_data_login_path_{id_device}.json', 'r', encoding='utf-8') as f:
    ...
    try:
        ...
        nope = ['Đã đăng nhập', "Thêm tài khoản",
                "Chuyển tài khoản", "Thêm tài khoản để đăng nhập nhanh."]
        elements = d(resourceId="com.zing.zalo:id/name")
        zalo_name = [zalo['name'] for zalo in zalo_data if "name" in zalo.keys()]
        for elem in elements:
            name_zalo_1 = elem.info.get('text', '').strip()
            if name_zalo_1 not in nope and name_zalo_1 not in zalo_name:
        ...
        for id in range(len(name_zalos)):
            ...
            with open(f"D:/Zalo_CRM/Zalo_base/Zalo_data_login_path_{id_device}.json", 'w', encoding="utf-8") as f:
                json.dump(zalo_data, f, ensure_ascii=False, indent=4)
            eventlet.sleep(5.0)

            if id < len(name_zalos)-1:
                d = switch_account(d, name_zalos[id+1])
                status = update_base_document_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{id_device}", {
    ...
    with open(f'D:/Zalo_CRM/Zalo_base/device_status_{id_device}.json', 'r') as f:
        device_status = json.load(f)
    d.app_start("com.zing.zalo", stop=True)
    eventlet.sleep(2.0)
    device_status['active'] = False
...
def background_get_unseen_loop():
    ...
    while True:
        ...
        on_chat = False
        # update = False
        docs = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {
...
def background_update_1vs1_loop():
    ...
    while True:
        ...
        document = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {
            "num_phone_zalo": now_phone_zalo})[0]
        id_device = document['id_device']
        documents = get_base_id_zalo_json(
            "D:/Zalo_CRM/Zalo_base", "id_device", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {"id_device": id_device})
        current_phone = ""
        for it in documents:
            if it['status']:
                current_phone = it['num_phone_zalo']
                ...
                try:
                    ...
                    if len(elems) > 0:
                        ...
                        if ck_noti:
                            ...
                            update_base_document_json(
                                "D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[curr_phone_zalo]}", data_update)
                    ...
                    print(name_ntd)
                    with open(f'D:/Zalo_CRM/Zalo_base/device_status_{dict_phone_device[curr_phone_zalo]}.json', 'r') as f:
                        device_status = json.load(f)
                    if device_status['active']:
                        ...
                        if name_ntd in name_group:
                            ...
                            try:
                                ...
                                print("Dữ liễu đoạn chat là:", chat_box_on_chat)
                            except Exception as e:
                            ...
                            if not chat_box_on_chat:
                                break
                            if chat_box_on_chat and len(chat_box_on_chat) > 0:
                                document = get_base_id_zalo_json("D:/Zalo_CRM/Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[curr_phone_zalo]}", {
...
if __name__ == "__main__":
    ...
    socketio.run(app, host="0.0.0.0", port=8016,
