...
def get_phone_numbers_from_api(emp_ids, size=1, get_fb_link=True):
    ...
    payload = {
        ...
        "size": 2, # số số điện thoại từ mã chuyên viên emp_id
        "key": API_KEY,
...
class DeviceHandler:
    ...
    def extract_profile_info(self, phone_number, original_info):
        ...
        try:
            ...
            log_message(f"[{DEVICE_LIST_NAME[self.device_id]}] Trích xuất thông tin thành công từ sđt {phone_number}")
            return profile_data
    ...
    def process_phone_number(self, phone_number, contact_info, sender_name, emp_id): # THÊM emp_id
        ...
        try:
            ...
            interacted, message, friend_or_not, name_zalo, name_ntd = self.send_message_add_friend(
            ...
            document = get_base_id_zalo_json("Zalo_base", "name", f"Zalo_data_login_path_{DEVICE_LIST_NAME[self.device_id]}", {
                "name": name_zalo})[0]
            #print("Phần tử được lấy ra là: ", document)
            print("Đã lấy file base thành công ", f"Zalo_data_login_path_{DEVICE_LIST_NAME[self.device_id]}")
            ...
            print("Đã lưu vào database ", f"Zalo_data_login_path_{DEVICE_LIST_NAME[self.device_id]}")
