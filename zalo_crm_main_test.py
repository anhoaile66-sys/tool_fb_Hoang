...
def get_list_emoji_sticker_u2(d: u2.Device, id_device="", max_emoji_sticker: int = 200, scroll_delay: float = 1.0, retire=3, has_update=False, friend_name=[], num_phone_zalo="", list_friend_old=[]):
    ...
    try:
        ...
        while len(emoji_sticker) < max_emoji_sticker:
            ...
            elems = d.xpath('//*[@resource-id="com.zing.zalo:id/emoticon_selector_grid"]/android.widget.LinearLayout')
            for ll in elems:
...
def get_data_chat_boxes_1vs1_u2(d: u2.Device, list_prior_chat_boxes, pick, max_scroll: int = 5, scroll_delay: float = 1.0):
    """
    Trả về dict chứa hộp chat hiện có trong màn hình chat của Zalo Android.
    - div_child_selector: dict để tìm tab/conversation (ví dụ {'resourceId': '...', 'text': 'Tên bạn'})
    - device_url: đường dẫn để kết nối uiautomator2 (ví dụ '192.168.1.100:5555')
    ...
    if num+1 > 0:
        dem = 0
        time_and_mes = []
        num_pr = len(list_prior_chat_boxes[pick]['data_chat_box'])-1
        for id in range(num_pr+1):
            if dem == num+1:
                break
            last_data = list_prior_chat_boxes[pick]['data_chat_box'][num_pr-id]
            last_data_mes = {}
            ck_you = False
            del_you = ""
            for it in last_data.keys():
                ...
                if dem == num+1:
                    break
                num_lt = len(last_data[it])-1
                for iy in range(num_lt+1):
                    last_data_mes = last_data[it][num_lt-iy]
                    if it == 'you':
                        print("Có tồn tại phần tử You")
                        del_you = last_data_mes['data']
                        ck_you = True
                        break
                    time_and_mes.append(
            ...
            if ck_you:
                break
        if dem == 0:
            time_and_mes.append({"time": "", "message": ""})
        #time_and_mes = time_and_mes[::-1]
        # Đọc từ dưới lên trên (các tin mới trước)
        now_mess = []
        for id in range(num+1):
            raw = items[num-id].text or ""
        ...
        for id in range(num+1):
            print("Có ổn không vậy cu")
            brk = False
            im = 0
            for il in range(len(now_mess)):
                if il < id:
                    continue

                if time_and_mes[im]['message'] != "" and now_mess[il] != "":
                    if time_and_mes[il]['message'] in now_mess[il] or now_mess[il] in time_and_mes[il]['message']:
                        brk = True
                        im += 1
                        if im == dem-1:
                            break
            ...
            raw = items[num-id].text or ""
            if del_you != "":
                if del_you in raw or raw in del_you:
                    break
            print("Raw là", raw)
            ...
            if time_and_mes['message'] in lines:
                if "[Hình ảnh]" not in time_and_mes['message'] and "[Sticker]" not in time_and_mes['message']:
...
def background_update_1vs1_loop(id_device):
    ...
    while True:
        ...
        try:
            with open(f'C:/Zalo_CRM/Zalo_base/device_status_{dict_phone_device[now_phone_zalo[id_device]]}.json', 'r') as f:
                device_status = json.load(f)
            if not device_status['update']:
