...
dict_devices = ["R5CW71JT8GT", "TSPNH6GYZLPJBY6X", "9PAM7DIFW87DOBEU", "2926294610DA007N", "7DXCUKKB6DVWDAQO", "7HYP4T4XTS4DXKCY", "CQIZKJ8P59AY7DHI", "CEIN4X45I7ZHFEFU", "8HMN4T9575HAQWLN", "UWJJOJLB85SO7LIZ"]
dict_status_zalo = {}
dict_status_update_pvp = {}
dict_phone_device = {}

# 2926294610DA007N"  # Máy 17
# 7DXCUKKB6DVWDAQO"  # Máy 9
# 7HYP4T4XTS4DXKCY"  # Máy 29
# CQIZKJ8P59AY7DHI"  # Máy 34
# CEIN4X45I7ZHFEFU"
dict_device_and_phone = {
    ...
    "IJP78949G69DKNHM": ['0987608429', '0337416995'],  #A16
    "EM4DYTEITCCYJNFU": ['0966117160', '0961963671', '0966578630'], #A17
    "PN59BMHYPFXCPN8T": [], #A18
    "Z5LVOF4PRGXGTS9H": ['0988658315'] #A19
}
...
def run_start(d: u2.Device):
    ...
    try:
        btn = d(text="Zalo")
        btn.click_exists(timeout=1)
    except Exception:
        pass
    return d


def create_android_driver(udid: str):
    if udid in drivers:
        return drivers[udid]
    ...
    opts.app_package = "com.zing.zalo"
    opts.app_activity = ".ui.SplashActivity"
    opts.no_reset = True
    opts.new_command_timeout = 300

    driver = appium_webdriver.Remote(executor, options=opts)
    drivers[udid] = driver
    return driver
...
def switch_account(d: u2.Device, name, retire=3):
    ...
    try:
        ...
        if retire > 0:
            d = switch_account(d, name, retire-1)
        else:
            return False
    eventlet.sleep(0.5)
...
def get_list_friends_u2(d: u2.Device, max_friends: int = 150, scroll_delay: float = 1.0, retire=3):
    """
...
def get_list_groups_u2(d: u2.Device, max_groups: int = 50, scroll_delay: float = 0.5, retire=3):
    """
...
def get_list_invite_friends_u2(d: u2.Device, max_friends: int = 100, scroll_delay: float = 0.5, retire=3):
    """
...
def handle_join(data):
    ...
    # res =  update_port_base_id_chat("Zalo_base", "Zalo_data_login_port",room)
...
def api_get_list_friend(data_body):
    ...
    docs = get_base_id_zalo_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            docs = get_base_id_zalo_json(
                "Zalo_base", "id_device", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {"id_device": id_device})
            check_busy = False
            for doc in docs:
            ...
            if check_busy:
                ...
                one = time.time()
                doc = get_base_id_zalo_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
                ...
                if not doc['status']:
                    ...
                    for it in docs:
                        if it['status']:
                            ...
                            break
                    if current_phone != "":
                        status = update_base_document_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {
                ...
                try:
                    result = get_list_friends_u2(d, max_friends=50)
            ...
            #     json.dump(dict_zalo_online, json_file, indent=4)
                    ...
                    update_base_document_json(
                        "Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", data_update)
...
def capture_and_store_avatars(user_name, scroll_times=5):
    # Kết nối thiết bị
    d = u2.connect("R5CW71JT8GT")
    d.app_start("com.zing.zalo", stop=True)
    time.sleep(0.5)

    btn = d(resourceId="com.zing.zalo:id/maintab_contact")
    if not btn.exists:
        btn = d(resourceId="com.zing.zalo:id/icon")
    btn.click()
    time.sleep(1)
...
#        img = Image.open(io.BytesIO(png))
        for id in range(num_item):
            ...
            items[id].click()
            time.sleep(1)
            elm_msg = d(resourceId="com.zing.zalo:id/action_bar_title")
            if elm_msg.exists:
                elm_msg.click()
            else:
                continue
            ...
            if not iv.exists:
                continue
        #    bounds = iv.info['bounds']
        #    crop = img.crop((bounds[0][0], bounds[0][1], bounds[1][0], bounds[1][1]))

            avatar_img = iv.screenshot()

        # 5) Lưu vào GridFS
            bio = io.BytesIO()
            avatar_img.save(bio, format="PNG")
            avatar_img.save(f"avatar_{user_name}.png")
            bio.seek(0)
            file_id = fs.put(
                bio.read(),
                filename=f"avatar_{user_name}.png",
                user_name=user_name
            )
            time.sleep(1)
            print("Stored avatar with _id:", file_id)
            d(resourceId="android:id/home").click()
            d(resourceId="android:id/home").click()
            if id < num_item - 1:
                items = d.xpath(
...
def get_list_prior_chat_boxes_u2(d: u2.Device, max_chat_boxes: int = 50, scroll_delay: float = 0.5, retire=3, scroll_or_not=True):
    """
    Lấy toàn bộ nhóm từ tab Danh bạ trên Zalo Android bằng uiautomator2.
    - d: uiautomator2 Device
    - max_groups: giới hạn tối đa số nhóm thu thập
...
def get_list_unseen_chat_boxes_u2(d: u2.Device, max_chat_boxes: int = 50, scroll_delay: float = 0.5, retire=3):
    """
    Lấy toàn bộ nhóm từ tab Danh bạ trên Zalo Android bằng uiautomator2.
    - d: uiautomator2 Device
    - max_groups: giới hạn tối đa số nhóm thu thập
...
def api_update_list_prior_chat_boxes(data_body, max_chat_boxes=1000, scroll_or_not=True):
    # data_body = request.form
    new_id = data_body['num_phone_zalo']
            ...
            if check_busy:
                ...
                try:
                    result = get_list_prior_chat_boxes_u2(
                        d, max_chat_boxes=max_chat_boxes, scroll_or_not=scroll_or_not)
                    if scroll_or_not:
...
# @app.route('/api_update_list_unseen_chat_boxes', methods=['POST', 'GET'])
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            if check_busy:
                ...
                try:
                    ...
                    try:
                        #        print(document)
                        with open(f'Zalo_base/Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'r', encoding='utf-8') as f:
                            data = json.load(f)
                        for id in range(len(data)):
                            #            print(document[domain])
...
#        print(data)
                        with open(f'Zalo_base/Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}.json', 'w', encoding='utf-8') as f:
                            json.dump(data, f, ensure_ascii=False, indent=4)
                            print(
...
def start_server(port):
    """Start a WSGI server listening on 0.0.0.0:port."""
    print(f"🚀 Starting server on port {port}")
    # Lưu ý: sử dụng socketio.server để để SocketIO work đúng
    eventlet.wsgi.server(eventlet.listen(('0.0.0.0', port)), socketio.server)

if __name__ == '__main__':
    # Danh sách các port muốn chạy server
    ports = [8001, 8002]

    # Spawn song song mỗi server
    for p in ports:
        eventlet.spawn_n(start_server, p)
'''
'''
def start_app(port):
...
if __name__ == '__main__':
    ...
    procs = []
    for p in ports:
        proc = Process(target=start_app, args=(p,))
        proc.start()
        procs.append(proc)

    # Tùy chọn: chờ cho các process kết thúc (thường không cần)
    for proc in procs:
        proc.join()
'''
'''
...
if __name__ == '__main__':
    ports = [8001, 8002]
    for p in ports:
        proc = multiprocessing.Process(target=start_app, args=(p,), daemon=True)
        proc.start()

    # Giữ main process chạy, hoặc dùng proc.join() nếu bạn muốn block ở đây
    for proc in multiprocessing.active_children():
        proc.join()
'''
'''
def start_server(port):
    # Ép xài IPv4 để không lấn sang IPv6 và không trùng socket
    listener = eventlet.listen(('0.0.0.0', port), family=socket.AF_INET)
    print(f"🚀 Socket.IO server listening on http://0.0.0.0:{port}")
    # Dùng trực tiếp WSGI server của eventlet
    eventlet.wsgi.server(listener, socketio.server)

if __name__ == '__main__':
    # Danh sách cổng muốn chạy song song
    ports = [8001, 8002]

    # Spawn green thread cho mỗi port
    for p in ports:
        eventlet.spawn_n(start_server, p)

    # Giữ cho main thread không exit
    eventlet.sleep()
'''
...
def handle_chat_pvp_v2(data):
    print(data)
    one = time.time()
    list_socket_call.append("open_chat_pvp_v2")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    num_send_phone_zalo = data['num_send_phone_zalo']
                    ...
                    if not on_chat:
                        ...
                        if not check_ex:
                            ...
                            # time.sleep(0.5)
                                ...
                                print(e)
                                dict_status_zalo[num_phone_zalo] = ""
                                emit("receive_chat_view_status_v2", {
                                     "status": "Mở chat thất bại, hãy thử mở lại", "name_ntd": name}, room=room)
                                return False
...
@app.route('/api_open_chat_pvp_test', methods=['POST', 'GET'])
def handle_chat_pvp_test():
    # first = time.time()
    # print(data)
    data = request.form
    first = time.time()
...
#    room = data["id_chat"]
    ...
    num_send_phone_zalo = "0389003999"
    global now_phone_zalo
    now_phone_zalo = num_phone_zalo
    one = time.time()
    # name = data.get('name')
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                if num_send_phone_zalo != "":
                    ...
                    for id in range(len(list_prior_chat_boxes)):
                        if "phone" not in list_prior_chat_boxes[id].keys():
                            list_prior_chat_boxes[id]['phone'] = ""
                        if list_prior_chat_boxes[id]['phone'] == num_send_phone_zalo:
                            name_ntd = list_prior_chat_boxes[id]['name']
                            # pick = id
                            break
                ...
                # dict_status_update_pvp[num_phone_zalo] = 2
                # handle_chat_view(num_phone_zalo)
                two = time.time()

                print("Lần 5 ", two-five)
                print(two-first)
                return jsonify({"Thời gian là ": two-first})
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                try:
                    msg = d(resourceId="com.zing.zalo:id/leading_container")
                    msg.click_exists(timeout=1)
                    #time.sleep(0.5)
                except Exception:
                    pass
                while True:
                    try:
                        home = d(resourceId="android:id/home")
                        home.click(timeout=1)
                    except Exception:
                        break
                try:
                    d(resourceId="com.zing.zalo:id/maintab_message").click()
                    #time.sleep(0.5)
                except Exception:
                    pass
                '''
                if num_send_phone_zalo != "":
                    ...
                    try:
                        ...
                        # emit("receive status",{"status":"Số điện thoại chưa tạo tài khoản zalo"},room=room)
                        return False
                    '''
                    try:
                        send_request = d(resourceId="com.zing.zalo:id/btn_send_friend_request").click()
                    ...
                    if not check:
                        list_prior_chat_boxes.append(
                            {"name": name_ntd, "time": "", "message": "", "status": "seen",  "data_chat_box": []})
                        data_update = {"num_phone_zalo": num_phone_zalo,
                                       "list_prior_chat_boxes": list_prior_chat_boxes}
                        update_base_document_json(
                            "Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", data_update)
                    ...
                    try:
                        ...
                        # emit("receive status",{"status":"Tài khoản không tồn tại"},room=room)
                        return False
...
def get_data_chat_boxes_gr_u2(d: u2.Device, time_and_mes, max_scroll: int = 5, scroll_delay: float = 1.0):
    """
    Trả về dict chứa hộp chat hiện có trong màn hình chat của Zalo Android.
    - div_child_selector: dict để tìm tab/conversation (ví dụ {'resourceId': '...', 'text': 'Tên bạn'})
    - device_url: đường dẫn để kết nối uiautomator2 (ví dụ '192.168.1.100:5555')
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        ...
        if dict_status_zalo[num_phone_zalo] != '' or dict_status_update_pvp[num_phone_zalo] != 0:
            ...
            if check_busy:
                ...
                raw_result = get_data_chat_boxes_u2(d, gr_or_pvp, time_and_mes)
        ...
        if dict_status_zalo[num_phone_zalo] != '':
            ...
            if check_busy:
                ...
                if not document['status']:
                     docs = get_base_id_zalo_json("Zalo_base", "id_device", "Zalo_data_login_path", {"id_device": id_device})
                     current_phone = ""
                     for it in docs:
                        if it['status']:
                            current_phone = it['num_phone_zalo']
                            break
                     if current_phone != "":
                        status = update_base_document_json("Zalo_base", "num_phone_zalo", "Zalo_data_login_path", {"num_phone_zalo": current_phone, "status" : False})
                     d = switch_account(d, user_name)
                     status = update_base_document_json("Zalo_base", "num_phone_zalo", "Zalo_data_login_path", {"num_phone_zalo": num_phone_zalo, "status" : True})
                '''
                # dict_status_zalo[num_phone_zalo] = "get_data_one_box_chat"
                if 'list_prior_chat_boxes' not in document.keys():
                    document['list_prior_chat_boxes'] = []
            ...
            if check_busy:
                ...
                if not check:
                    ...
                    if 'data_chat_box' not in list_prior_chat_boxes[pick].keys() or list_prior_chat_boxes[pick]['data_chat_box'] == []:
                        ...
                        time_and_mes = {
                            "time": last_data_mes['time'], "message": last_data_mes['data']}
                raw_result = get_data_chat_boxes_gr_u2(d, time_and_mes)
                result = [r for r in raw_result if r]
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                print("bận rồi !!!", dict_status_zalo[num_phone_zalo])
                emit("busy", {
                     "status": dict_status_zalo[num_phone_zalo], "name_ntd": name, "message": message}, room=room)
                return False
            else:
                dict_status_update_pvp[num_phone_zalo] = 1
                '''
                ...
                if type == 'image':
                    ...
                    with open(f'Zalo_base/image{image_number}.jpg', 'wb') as f:
                        f.write(avatar)
                    print(
                        f"Đã lưu vào thư mục Zalo_base/image{image_number}.jpg")
                    d.push(f'Zalo_base/image{image_number}.jpg',
                           f'/sdcard/Download/image{image_number}.jpg')
                    # d.push('Zalo_base/image.jpg', f'/sdcard/DCIM/Zalo/image{image_number}.jpg')
                    # d.push('Zalo_base/image.jpg', f'/sdcard/Pictures/Zalo/image{image_number}.jpg')
                    eventlet.sleep(0.1)
                    image_number -= 1
                    ...
                    if "word" in file_type:
                        with open(f'Zalo_base/{file_name}.docx', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'Zalo_base/{file_name}.docx', f'/sdcard/Download/{file_name}.docx')
                    elif "pdf" in file_type:
                        with open(f'Zalo_base/{file_name}.pdf', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'Zalo_base/{file_name}.pdf', f'/sdcard/Download/{file_name}.pdf')
                    elif "sheet" in file_type:
                        with open(f'Zalo_base/{file_name}.xlsx', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'Zalo_base/{file_name}.xlsx', f'/sdcard/Download/xlsx{image_number}.xlsx')
                    elif "present" in file_type:
                        with open(f'Zalo_base/present{image_number}.ppt', 'wb') as f:
                           f.write(file_decode)
                        d.push(f'Zalo_base/present{image_number}.ppt', f'/sdcard/Download/present{image_number}.ppt')
                    '''
                    with open(f'Zalo_base/{file_name}', 'wb') as f:
                        f.write(file_decode)
                    d.push(f'Zalo_base/{file_name}',
...
# docs giờ là một list chứa mọi document tìm được
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            print("bận rồi !!!", dict_status_zalo[num_phone_zalo])
            emit("busy", {
                 "status": dict_status_zalo[num_phone_zalo],  "name_ntd": names}, room=room)
            return False
        else:
            docs = get_base_id_zalo_json(
                "Zalo_base", "id_device", f"Zalo_data_login_path_{dict_phone_device[num_phone_zalo]}", {"id_device": id_device})
            ...
            if check_busy:
                print("bận rồi !!!", dict_status_zalo[num_phone_zalo])
                emit("busy", {
                     "status": dict_status_zalo[num_phone_zalo], "name_ntd": names, "message": message}, room=room)
                return False
            else:
                dict_status_update_pvp[num_phone_zalo] = 1
...
@socketio.on('click_send_card_chat_pvp')
def handle_click_send_card_chat_pvp(data):
    list_socket_call.append("click_send_card_chat_pvp")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    # num_send_phone_zalo = data['num_send_phone_zalo']
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                print("bận rồi !!!", dict_status_zalo[num_phone_zalo])
                emit("busy", {
                     "status": dict_status_zalo[num_phone_zalo], "name_ntd": name, "name_ntd": name}, room=room)
                return False
            else:
                dict_status_update_pvp[num_phone_zalo] = 1
                dict_status_zalo[num_phone_zalo] = "click_send_card_chat_pvp"
                # if d is None:
                d = u2.connect(id_device)
                try:
                    d(resourceId="com.zing.zalo:id/new_chat_input_btn_attach").click_exists(timeout=1)
                    d.xpath('//*[@text="Danh thiếp"]').click_exists(timeout=1)
                except Exception as e:
                    print(e)
                    dict_status_zalo[num_phone_zalo] = ""
                    emit("receive_click_send_card_status", {
                         "status": "Mở danh thiếp thất bại",  "name_ntd": name}, room=room)
                    return False

                eventlet.sleep(0.5)
                join_room(room)
                emit("receive_click_send_card_status", {
                     "status": "Đã mở danh thiếp thành công"}, room=room)
                dict_status_zalo[num_phone_zalo] = ""
                num_message += 1
                dict_status_update_pvp[num_phone_zalo] = 2
                last_time[num_phone_zalo] = time.time()
...
@socketio.on('click_stop_send_card_chat_pvp')
def handle_stop_send_card_chat_pvp(data):
    list_socket_call.append("click_stop_send_card_chat_pvp")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    # num_send_phone_zalo = data['num_send_phone_zalo']
        ...
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                try:
                    ...
                    print(e)
                    dict_status_zalo[num_phone_zalo] = ""
                    emit("receive_stop_send_card_status", {
                         "status": "Dừng gửi thiệp thất bại",  "name_ntd": name}, room=room)
                    return False

                # eventlet.sleep(0.5)
                join_room(room)
                emit("receive_stop_send_card_status", {
                     "status": "Đã hủy gửi thiệp thành công"}, room=room)
                dict_status_zalo[num_phone_zalo] = ""
                # num_message += 1
                dict_status_update_pvp[num_phone_zalo] = 2
                last_time[num_phone_zalo] = time.time()
...
@socketio.on('select_send_card_chat_pvp')
def handle_select_send_card_chat_pvp(data):
    list_socket_call.append("select_send_card_chat_pvp")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                dict_status_update_pvp[num_phone_zalo] = 1
                dict_status_zalo[num_phone_zalo] = "select_send_card_chat_pvp"
                # if d is None:
                d = u2.connect(id_device)
                try:
                    d(resourceId="com.zing.zalo:id/search_input_text").click_exists(timeout=1)
                    eventlet.sleep(0.1)
                    d.send_keys(card_name, clear=True)
                    eventlet.sleep(0.1)
                    d.xpath(
                        '//*[@resource-id="com.zing.zalo:id/phoneList"]/android.widget.LinearLayout[1]').click_exists(timeout=1)
                    eventlet.sleep(0.1)
                except Exception as e:
                    print(e)
                    dict_status_zalo[num_phone_zalo] = ""
                    emit("receive_select_send_card_status", {
                         "status": "Chọn danh thiếp thất bại",  "name_ntd": name}, room=room)
                    return False

                # eventlet.sleep(0.5)
                join_room(room)
                emit("receive_select_send_card_status", {
                     "status": "Đã chọn danh thiếp thành công"}, room=room)
                dict_status_zalo[num_phone_zalo] = ""
                # num_message += 1
                dict_status_update_pvp[num_phone_zalo] = 2
                last_time[num_phone_zalo] = time.time()
...
@socketio.on('remove_send_card_chat_pvp')
def handle_remove_send_card_chat_pvp(data):
    list_socket_call.append("remove_send_card_chat_pvp")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                dict_status_update_pvp[num_phone_zalo] = 1
                dict_status_zalo[num_phone_zalo] = "remove_send_card_chat_pvp"
                # if d is None:
                d = u2.connect(id_device)
                try:
                    d.xpath(
                        f'//*[@resource-id="com.zing.zalo:id/rv_bubbles"]/android.widget.FrameLayout[{id_card}]/android.view.View[1]').click_exists(timeout=1)
                except Exception as e:
                    print(e)
                    dict_status_zalo[num_phone_zalo] = ""
                    emit("receive_remove_send_card_status", {
                         "status": "Bỏ chọn danh thiếp thất bại",  "name_ntd": name}, room=room)
                    return False

                # eventlet.sleep(0.5)
                join_room(room)
                emit("receive_remove_send_card_status", {
                     "status": "Bỏ chọn danh thiếp thành công"}, room=room)
                dict_status_zalo[num_phone_zalo] = ""
                # num_message += 1
                dict_status_update_pvp[num_phone_zalo] = 2
                last_time[num_phone_zalo] = time.time()
...
@socketio.on('send_card_chat_pvp')
def handle_send_card_chat_pvp(data):
    list_socket_call.append("send_card_chat_pvp")
#    room = data["id_chat"]
    room = data['id_chat']
    num_phone_zalo = data['num_phone_zalo']
    id_card = data['id_card']
    ...
    if (id_device in dict_devices and num_phone_zalo in dict_status_zalo.keys()):
        if (dict_status_zalo[num_phone_zalo] != ''):
            ...
            if check_busy:
                ...
                dict_status_update_pvp[num_phone_zalo] = 1
                dict_status_zalo[num_phone_zalo] = "send_card_chat_pvp"
                # if d is None:
                d = u2.connect(id_device)
                try:
                    d(resourceId="com.zing.zalo:id/btn_done_add_item").click_exists(timeout=1)
                except Exception as e:
                    print(e)
                    dict_status_zalo[num_phone_zalo] = ""
                    emit("receive_send_card_status", {
                         "status": "Gửi danh thiếp thất bại",  "name_ntd": name}, room=room)
                    return False

                # eventlet.sleep(0.5)
                join_room(room)
                emit("receive_send_card_status", {
                     "status": "Gửi danh thiếp thành công"}, room=room)
                dict_status_zalo[num_phone_zalo] = ""
                # num_message += 1
                dict_status_update_pvp[num_phone_zalo] = 2
                last_time[num_phone_zalo] = time.time()
...
def api_log_in_status(id_device):
    ...
    d(resourceId="com.zing.zalo:id/maintab_metab").click()
    eventlet.sleep(0.5)
    name_zalo = d(resourceId="com.zing.zalo:id/title_list_me_tab").get_text()
    docs = get_base_id_zalo_json(
        "Zalo_base", "id_device", f"Zalo_data_login_path_{id_device}", {"id_device": id_device})
    list_not_online = []
    online_zalo = ""

    for doc in docs:
        num_phone_zalo = doc['num_phone_zalo']
        if name_zalo == doc['name']:
            status = update_base_document_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{id_device}", {
                "num_phone_zalo": num_phone_zalo, "status": True})
            online_zalo = num_phone_zalo
        else:
            status = update_base_document_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{id_device}", {
                "num_phone_zalo": num_phone_zalo, "status": False})
            list_not_online.append(num_phone_zalo)
    num_zalos = []
    if online_zalo != "":
        num_zalos.append(online_zalo)
    for num in list_not_online:
        num_zalos.append(num)
    for num_phone_zalo in num_zalos:

        print("Bắt đầu lấy danh sách bạn bè")
        result1 = api_get_list_friend({"num_phone_zalo": num_phone_zalo})
        ...
        # result3 = api_get_list_invite_friend({"num_phone_zalo": num_phone_zalo})
        ...
        try:
            ...
            pass
            # elif name_ntd in result2:
            #   result6 = api_update_data_one_chat_box({"num_phone_zalo": num_phone_zalo, "name_ntd": name_ntd}, gr_or_pvp="gr", update=True)
            # else:
            #   result6 = api_update_data_one_chat_box({"num_phone_zalo": num_phone_zalo, "name_ntd": name_ntd}, update=True)
...
def background_get_unseen_loop():
    global now_phone_zalo
    global id_chat
    ...
    while True:
        ...
        on_chat = False
        # update = False
        docs = get_base_id_zalo_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {
                                     "num_phone_zalo": now_phone_zalo})
        if len(docs) > 0:
            document = docs[0]
        else:
            print(now_phone_zalo)
        # docs giờ là một list chứa mọi document tìm được
        id_device = document['id_device']
        ...
        if btn.exists:
            ...
            try:
                ...
                if name_ntd in name_group:
                    try:
                        chat_box_on_chat = api_update_data_one_chat_box(
                            {"num_phone_zalo": now_phone_zalo, "name_ntd": name_ntd}, gr_or_pvp="gr", on_chat=True)
                        on_chat = True
                    except Exception:
                        dict_status_zalo[now_phone_zalo] = ""
                if dict_status_update_pvp[now_phone_zalo] == 0:
                    # chat_box_on_chat = api_update_data_one_chat_box({"num_phone_zalo": now_phone_zalo, "name_ntd": name_ntd}, on_chat=True)
                    on_chat = True
            except Exception:
                pass
            '''
            ...
            on_chat = True
        print("Vào chưa")
        list_new_mess = api_update_list_unseen_chat_boxes(
            {'num_phone_zalo': now_phone_zalo})
        if list_new_mess and len(list_new_mess) > 0:
            num_new = len(list_new_mess)
            # socketio.emit('receive_new_data_update', {
            #              'status': "Có dữ liệu mới cập nhật"}, to=id_chat)
            while id < num_new:
                ...
                try:
                    '''
                    if list_new_mess[id] in name_group:
                        chat = api_update_data_one_chat_box(
                            {"num_phone_zalo": now_phone_zalo, "name_ntd": list_new_mess[id]}, gr_or_pvp="gr")
                    '''
                    '''
                    else:
                        chat = api_update_data_one_chat_box(
                            {"num_phone_zalo": now_phone_zalo, "name_ntd": list_new_mess[id]})
                    '''
                    pass
                except Exception as e:
                    print(e)
                    dict_status_zalo[now_phone_zalo] = ""
                if not chat or list_new_mess[id] not in name_group:
                    id += 1
                    continue
        ...
        if on_chat:
            ...
            while True:
                status = open_again_chat_pvp(
                    {'name': name_ntd, 'num_phone_zalo': now_phone_zalo, "num_send_phone_zalo": ""})
                if dict_status_zalo[now_phone_zalo] == 'handle_chat_pvp':
                    dict_status_zalo[now_phone_zalo] = ""
                if status or dict_status_zalo[now_phone_zalo] != "":
                    break
            # if chat_box_on_chat and len(chat_box_on_chat) > 0:
            #    socketio.emit('receive_new_message_from_ntd', {
            #                  'status': "Có dữ liệu mới từ khách hàng gửi đến"}, to=id_chat)
...
def background_update_1vs1_loop():
    # Test thôi, nhớ bỏ dòng này đi
    global now_phone_zalo
    global id_chat
    # now_phone_zalo = "0867956826"
    while True:
        ...
        document = get_base_id_zalo_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {
            "num_phone_zalo": now_phone_zalo})[0]
        id_device = document['id_device']
        documents = get_base_id_zalo_json(
            "Zalo_base", "id_device", f"Zalo_data_login_path_{dict_phone_device[now_phone_zalo]}", {"id_device": id_device})
        current_phone = ""
        for it in documents:
            if it['status']:
                current_phone = it['num_phone_zalo']
                if current_phone != "" and current_phone != now_phone_zalo:
                    now_phone_zalo = current_phone
                    document = it
                break
        ...
        user_name = document['name']
        # list_prior_chat_boxes = document['list_prior_chat_boxes']
        curr_phone_zalo = now_phone_zalo
        ...
        while True:
            ...
            if curr_phone_zalo != now_phone_zalo:
                print("Số điện thoại đã bị thay đổi: ", now_phone_zalo)
                break
                ...
                try:
                    ...
                    if update_d(text="Đã gửi lời mời kết bạn").exists:
                        ...
                        if btn.exists or kb.exists:
                            ...
                            friend_or_not = "yes"
                    '''
                    noti_driver = create_android_driver(id_device)
                    content_description = "Thông báo của Zalo"
                    locator = f'new UiSelector().descriptionContains("{content_description}")'
                    element = noti_driver.find_element_by_android_uiautomator(locator)
                    desc_value = element.get_attribute("name")
                    print("Nội dung content-desc là:", desc_value)
                    new_name_ntd = desc_value.replace("Thông báo của Zalo")
                    ...
                    list_prior_chat_boxes = document['list_prior_chat_boxes']
                    # test = update_d(
                    #    descriptionContains="Thông báo của Zalo")
                    # if test.exists:
                    #    print("Có tồn tại phần tử thông báo test")
                    ...
                    if len(elems) > 0:
                        ...
                        if ck_noti:
                            ...
                            update_base_document_json(
                                "Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[curr_phone_zalo]}", data_update)
                    ...
                    if name_ntd in name_group:
                        try:
                            if curr_phone_zalo != now_phone_zalo:
                                print(
                                    "Số điện thoại hiện tại đã bị thay đổi: ", now_phone_zalo)
                                break
                            chat_box_on_chat = api_update_data_one_chat_box(
                                {"num_phone_zalo": curr_phone_zalo, "name_ntd": name_ntd}, gr_or_pvp="gr", on_chat=True)
                        except Exception:
                            dict_status_zalo[curr_phone_zalo] = ""
                        if not chat_box_on_chat:
                            break
                        if chat_box_on_chat and len(chat_box_on_chat) > 0:
                            document = get_base_id_zalo_json("Zalo_base", "num_phone_zalo", f"Zalo_data_login_path_{dict_phone_device[curr_phone_zalo]}", {
                ...
                for id in range(len(list_prior_chat_boxes)):
                    if boxes[it]['name'] == list_prior_chat_boxes[id]['name']:
                        ...
                        '''
                            ...
                            if boxes[it]['time'] != list_prior_chat_boxes[id]['time']:
                                list_prior_chat_boxes[id]['time'] = boxes[it]['time']
                                list_prior_chat_boxes[id]['message'] = boxes[it]['message']
                                list_prior_chat_boxes[id]['status'] = "unseen"
                                list_prior_chat_boxes.insert(
                                    0, list_prior_chat_boxes.pop(id))
                                data_update = {"num_phone_zalo": now_phone_zalo,
                                               "list_prior_chat_boxes": list_prior_chat_boxes}
                                ck_noti = True
                        '''
                        break
...
if __name__ == "__main__":
    ...
    t.start()
    '''
    # id_device = "7DXCUKKB6DVWDAQO"
    # result  = api_log_in_status(id_device)
    # socketio.start_background_task(background_get_unseen_loop)
    # socketio.start_background_task(background_update_1vs1_loop)
